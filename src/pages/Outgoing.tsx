...
export const OutgoingPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error("Error submitting outgoing", err);
            alert("حدث خطأ أثناء تسجيل السحب");
        }
    };
            ...
            <AnimatePresence>
                {isSuccess && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="absolute top-0 right-0 left-0 mx-auto w-fit z-50 bg-green-500/90 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-xl backdrop-blur-md"
                    >
                        <CheckCircle size={20} /> تمت إضافة السحب بنجاح
                    </motion.div>
                )}
            </AnimatePresence>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="px-1 sm:px-0">
                    <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">سحب مخالات</h2>
                    <p className="text-gray-400 text-sm sm:text-base">تسجيل الصناديق التي تم سحبها من قبل المستخدمين.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1">
                    <form onSubmit={handleSubmit} className="glass-card p-4 sm:p-6 flex flex-col gap-6 lg:sticky lg:top-10">
                        <h3 className="text-xl font-bold">إضافة عملية سحب</h3>
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex flex-col gap-2 relative">
                                ...
                                <input
                                    ...
                                    className={cn(
                                        ...
                                        currentUser ? "focus:ring-2 focus:ring-green-500/50 border-green-500/30" : (userCode ? "focus:ring-2 focus:ring-red-500/50 border-red-500/30" : "focus:ring-2 focus:ring-brand-primary/50")
                                    )}
                                />
                                <AnimatePresence>
                                        ...
                                        >
                                            {suggestions.map(u => (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-brand-primary text-xs bg-brand-primary/10 px-2 py-1 rounded-md font-mono font-bold">{u.shortCode}</span>
                                                </div>
                                            ))}
                                        </motion.div>
                                    )}
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-xs text-gray-500 font-bold uppercase mr-1">الكمية</label>
                                <input
                                    type="number"
                                    value={quantity}
                                    ...
                                    required
                                    min="1"
                                    className="bg-black/20 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-primary/50 focus:border-transparent transition-all text-xl"
                                />
                            </div>
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="flex items-center gap-2 cursor-pointer text-gray-400 hover:text-white transition-colors w-fit select-none">
                                    <input 
                                        type="checkbox" 
                                        className="w-4 h-4 rounded border-white/10 bg-black/20 text-brand-primary focus:ring-brand-primary"
                                    ...
                                    {showCustomDate && (
                                        ...
                                        >
                                            <input
                                                ...
                                                className="bg-black/20 border border-white/5 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-brand-primary/50 focus:border-transparent transition-all text-gray-300 w-full"
                                            />
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            ...
                            <button type="submit" className="premium-gradient w-full py-4 rounded-xl font-bold shadow-glow mt-4 flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.02] transition-transform">
                                <Plus size={20} /> تسجيل السحب
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="glass-card p-6 flex flex-col gap-6">
                        <h3 className="text-xl font-bold">عمليات السحب الأخيرة</h3>
                        ...
                        <div className="flex flex-col gap-1 px-1">
                            {isLoading ? (
                                ...
                                <p className="text-gray-500 text-center py-8">لا توجد عمليات سحب مسجلة</p>
                            ) : (
                                transactions.map((tx) => (
                                    <div key={tx.id} className="flex items-center justify-between py-4 border-b border-white/5 last:border-0 hover:bg-white/5 px-4 -mx-4 rounded-xl transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-full bg-brand-primary/10 text-brand-primary flex items-center justify-center font-mono font-bold text-sm">
                                                {tx.userCode}
                                            </div>
                                            <div>
                                        ...
                                        <div className="text-left font-bold text-xl text-brand-primary flex flex-col items-end">
                                            - {tx.quantity} مخالة
